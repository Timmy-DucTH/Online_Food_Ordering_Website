...
const AdminDashboard = () => {
  ...
  // Kiểu dáng Table mẫu dùng chung
  const tableThStyle = { padding: '12px 15px', borderBottom: '2px solid #e2e8f0', color: '#475569', fontSize: '14px', fontWeight: '600' };
  const tableTdStyle = { padding: '12px 15px', borderBottom: '1px solid #e2e8f0', color: '#334155', fontSize: '14px' };
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f8fafc', color: '#1e293b', fontFamily: 'system-ui, sans-serif', textAlign: 'left' }}>
      ...
      {successMsg && (
        <div style={{ position: 'fixed', top: '25px', right: '25px', backgroundColor: '#0f172a', color: '#ffffff', borderLeft: '4px solid #10b981', padding: '15px 25px', borderRadius: '4px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', zIndex: 99999, fontSize: '14px', fontWeight: '500' }}>
          {successMsg}
        </div>
      )}

      {/* 🟢 SIDEBAR - THANH QUẢN TRỊ BÊN TRÁI */}
      <div style={{ width: '260px', backgroundColor: '#ffffff', borderRight: '1px solid #e2e8f0', padding: '25px 15px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'sticky', top: 0, height: '100vh', boxSizing: 'border-box' }}>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <button onClick={() => setActiveTab('overview')} style={{ width: '100%', padding: '12px 15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', textAlign: 'left', backgroundColor: activeTab === 'overview' ? '#10b981' : 'transparent', color: activeTab === 'overview' ? 'white' : '#64748b', transition: 'all 0.2s' }}>
              📊 Tổng Quan Hệ Thống
            </button>
            <button onClick={() => setActiveTab('users')} style={{ width: '100%', padding: '12px 15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', textAlign: 'left', backgroundColor: activeTab === 'users' ? '#10b981' : 'transparent', color: activeTab === 'users' ? 'white' : '#64748b', transition: 'all 0.2s' }}>
              👥 Quản Lý Người Dùng
            </button>
            <button onClick={() => setActiveTab('foods')} style={{ width: '100%', padding: '12px 15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', textAlign: 'left', backgroundColor: activeTab === 'foods' ? '#10b981' : 'transparent', color: activeTab === 'foods' ? 'white' : '#64748b', transition: 'all 0.2s' }}>
              🍔 Quản Lý Món Ăn
            </button>
            <button onClick={() => setActiveTab('orders')} style={{ width: '100%', padding: '12px 15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', textAlign: 'left', backgroundColor: activeTab === 'orders' ? '#10b981' : 'transparent', color: activeTab === 'orders' ? 'white' : '#64748b', transition: 'all 0.2s' }}>
              📦 Quản Lý Đơn Hàng
            </button>
          </div>
        </div>
      ...
      <div style={{ flex: 1, padding: '40px 30px', overflowY: 'auto', boxSizing: 'border-box' }}>
        ...
        {errorMsg && (
          <div style={{ padding: '15px', backgroundColor: '#fef2f2', border: '1px solid #fca5a5', color: '#b91c1c', borderRadius: '8px', marginBottom: '25px', fontSize: '14px', fontWeight: '500' }}>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', marginBottom: '30px' }}>
              <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                <span style={{ fontSize: '13px', fontWeight: 'bold', color: '#64748b' }}>💰 DOANH THU THỰC TẾ (CHƯA TÍNH HUỶ)</span>
              ...
              <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                ...
                <h2 style={{ fontSize: '28px', color: '#0f172a', margin: '10px 0 0 0', fontWeight: '800' }}>{activeOrdersCount} đơn</h2>
              </div>
              <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                <span style={{ fontSize: '13px', fontWeight: 'bold', color: '#64748b' }}>👥 TÀI KHOẢN THÀNH VIÊN</span>
                <h2 style={{ fontSize: '28px', color: '#0f172a', margin: '10px 0 0 0', fontWeight: '800' }}>{totalUsersCount} người</h2>
              </div>
              <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #fff1f2' }}>
            ...
            <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ margin: '0 0 15px 0', color: '#0f172a' }}>📈 Quy chế giám sát Middleware</h3>
        ...
        {activeTab === 'users' && (
          <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ margin: '0 0 6px 0' }}>👥 Danh sách Thành viên & Điểm uy tín</h3>
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '20px' }}>Theo dõi điểm uy tín thưởng/phạt và khóa các tài khoản vi phạm chính sách TasteByte.</p>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                ...
                <tbody>
                  {users.length === 0 ? (
                    <tr><td colSpan="6" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>Không tìm thấy người dùng nào trong cơ sở dữ liệu.</td></tr>
                  ) : (
                    users.map((user) => (
                      <tr key={user._id}>
                        ...
                        <td style={tableTdStyle}>
                          <span style={{ fontWeight: 'bold', color: user.credit_score < 50 ? '#e11d48' : '#10b981', backgroundColor: user.credit_score < 50 ? '#fff1f2' : '#e6f4ea', padding: '4px 10px', borderRadius: '6px' }}>
                            {user.credit_score ?? 100} điểm
                          </span>
                        </td>
                        <td style={tableTdStyle}>
                          <span style={{ fontSize: '12px', padding: '3px 8px', borderRadius: '12px', fontWeight: 'bold', backgroundColor: user.status === 'banned' ? '#fff1f2' : '#e6f4ea', color: user.status === 'banned' ? '#e11d48' : '#059669' }}>
                        ...
                        <td style={tableTdStyle}>
                          <button 
                            ...
                            style={{ padding: '6px 12px', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: user.status === 'banned' ? '#10b981' : '#fff1f2', color: user.status === 'banned' ? 'white' : '#e11d48', transition: 'all 0.2s' }}
                          >
        ...
        {activeTab === 'foods' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            ...
            <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ margin: '0 0 15px 0' }}>➕ Thêm Món Ăn Mới Vào Thực Đơn</h3>
              <form onSubmit={handleAddFoodSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <input type="text" placeholder="Tên món ăn (Ví dụ: Cơm gà Hải Nam)" value={newFood.name} onChange={(e) => setNewFood({ ...newFood, name: e.target.value })} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', outline: 'none' }} required />
                <input type="number" placeholder="Giá tiền bán (VNĐ)" value={newFood.price} onChange={(e) => setNewFood({ ...newFood, price: e.target.value })} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', outline: 'none' }} required />
                <select value={newFood.category} onChange={(e) => setNewFood({ ...newFood, category: e.target.value })} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', backgroundColor: 'white' }} required>
                  <option value="">-- Chọn danh mục món ăn --</option>
                  <option value="Đồ ăn nhanh">Đồ ăn nhanh</option>
                  <option value="Món nước">Món nước</option>
                  <option value="Cơm văn phòng">Cơm văn phòng</option>
                  <option value="Trà sữa & Đồ uống">Trà sữa & Đồ uống</option>
                </select>
                <input type="text" placeholder="URL ảnh minh họa món ăn" value={newFood.image} onChange={(e) => setNewFood({ ...newFood, image: e.target.value })} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', outline: 'none' }} required />
                <input type="text" placeholder="Mô tả ngắn thành phần dinh dưỡng" value={newFood.description} onChange={(e) => setNewFood({ ...newFood, description: e.target.value })} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', gridColumn: '1 / span 2', outline: 'none' }} />
                <button type="submit" style={{ gridColumn: '1 / span 2', padding: '12px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>
            ...
            <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <h3 style={{ margin: '0 0 15px 0' }}>🍔 Danh mục thực đơn hiện hành</h3>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {foods.length === 0 ? (
                      <tr><td colSpan="5" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>Chưa có món ăn nào trong database. Hãy thêm món đầu tiên!</td></tr>
                    ) : (
                      foods.map((food) => (
                        <tr key={food._id}>
                          ...
                          <td style={tableTdStyle}><span style={{ backgroundColor: '#f1f5f9', padding: '3px 8px', borderRadius: '6px', fontSize: '13px' }}>{food.category}</span></td>
                          <td style={tableTdStyle}><span style={{ color: '#059669', fontWeight: '600' }}>{Number(food.price).toLocaleString('vi-VN')}đ</span></td>
                          <td style={tableTdStyle}>
                            <button onClick={() => handleDeleteFood(food._id)} style={{ padding: '5px 10px', backgroundColor: '#fff1f2', color: '#e11d48', border: '1px solid #ffe4e6', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
                              🗑️ Xóa món
                            </button>
                          </td>
                        </tr>
        ...
        {activeTab === 'orders' && (
          <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ margin: '0 0 6px 0' }}>📦 Quản lý dòng vận đơn giao hàng</h3>
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '20px' }}>Kiểm duyệt trạng thái luồng vận chuyển thức ăn. Nếu khách hàng cố tình bùng đơn, chuyển sang trạng thái Canceled để kích hoạt trừ điểm uy tín.</p>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {orders.length === 0 ? (
                    <tr><td colSpan="6" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>Hệ thống chưa phát sinh bất kỳ đơn đặt hàng nào.</td></tr>
                  ) : (
                    orders.map((order) => (
                      <tr key={order._id}>
                        <td style={tableTdStyle}><code style={{ color: '#0f172a', fontWeight: 'bold' }}>#{order._id?.substring(18)}</code></td>
                        ...
                        <td style={tableTdStyle}><span style={{ fontSize: '13px', color: '#475569' }}>{order.address || 'Tại quầy cửa hàng'}</span></td>
                        <td style={tableTdStyle}>
                          <span style={{ 
                            padding: '4px 8px', borderRadius: '12px', fontSize: '12px', fontWeight: 'bold',
                            backgroundColor: order.status === 'completed' ? '#e6f4ea' : order.status === 'pending' ? '#fef3c7' : '#fff1f2',
                            color: order.status === 'completed' ? '#059669' : order.status === 'pending' ? '#d97706' : '#e11d48'
                          }}>
                        ...
                        <td style={tableTdStyle}>
                          <select 
                            ...
                            style={{ padding: '6px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '13px', backgroundColor: 'white', cursor: 'pointer' }}
                          >
                            <option value="pending">⌛ Chờ xử lý</option>
                            <option value="completed">✓ Đã giao hàng</option>
                            <option value="cancelled">❌ Hủy / Bùng hàng (Trừ uy tín)</option>
                          </select>
                        </td>
                      </tr>
                    ))
